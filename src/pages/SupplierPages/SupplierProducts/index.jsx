...
const SupplierProducts = () => {
  ...
  } = useGetAllProductsBySearchCategoryQuery(searchCategory, {
    skip: !searchCategory,
  });
  const {
    data: searchResultsName,
    refetch: refetchSearchName,
    isFetching: isSearchingName,
    isLoading: isLoadingSearchName,
  ...
  const filteredProducts =
    searchCategory && searchName
      ? // hər iki search aktivdirsə → kəsişmə (intersection)
        (searchResults?.data || []).filter((c) =>
          (searchResultsName?.data || []).some((n) => n.id === c.id)
        )
      : searchCategory
      ? searchResults?.data || [] // yalnız kateqoriya search aktivdirsə
      : searchName
      ? searchResultsName?.data || [] // yalnız ad search aktivdirsə
      : productList || []; // heç biri aktiv deyilsə - default list
  ...
  const filteredDeleteRequests =
    deleteRequest?.filter((item) => item.deleted === true) || [];

  const combinedRequests = [
    ...filteredAddRequests.map((item) => ({ ...item, statusType: "add" })),
    ...filteredDeleteRequests.map((item) => ({
      ...item,
      statusType: "delete",
    })),
  ];

  const { data: getProductUpdateMyPending } =
    useGetProductUpdateMyPendingQuery();
      ...
      <div className="supplier-product">
        <div className={"headerr"}>
          <div className={"head"}>
            <h2>Məhsullar</h2>
            <p>
              Məhsul siyahısına baxın, dəyişikliklər edin və yeni məhsullar
              yaradın.
            </p>
          </div>
          <div>
                        ...
                        <tr key={i}>
                          ...
                          <td>
                            ...
                            >
                              ...
                              <svg
                                onClick={() => setDeleteIndex(item.id)}
                                style={{ cursor: "pointer" }}
                                xmlns="http://www.w3.org/2000/svg"
                                width="20"
                                height="20"
                            ...
                            >
                              {isConfirmed ? (
                                ...
                                >
                                  ...
                                  >
                                    <path
                                      d="M5.56836 12.9324L9.50061 9.00012M9.50061 9.00012L13.4329 5.06787M9.50061 9.00012L5.56836 5.06787M9.50061 9.00012L13.4329 12.9324"
                                      stroke="#ED0303"
                                      stroke-width="1.5"
                                      stroke-linecap="round"
                                      stroke-linejoin="round"
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  setModalData(null);
                  showPopup(
                    "Məhsul düzəliş tələbi göndərildi",
                    "Seçilmiş məhsul üzrə redaktə tələbiniz uğurla göndərildi və baxılmaq üçün sistemə daxil edildi.",
                    "success"
                  );
                } catch {
                  showPopup(
        ...
        <div className="modal-overlay" onClick={() => setDeleteIndex(null)}>
          ...
          >
            ...
            <p className="delete-message">
              Məhsulun silinməsi üçün administratora bildiriş göndəriləcək.
            </p>
            <p className="delete-sub">
              Silinmə yalnız təsdiqdən sonra həyata keçiriləcək.
            </p>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    setDeleteIndex(null);
                    showPopup(
                      "Məhsulun silinmə tələbi göndərildi",
                      "Seçilmiş məhsul üzrə silinmə tələbiniz uğurla göndərildi və baxılmaq üçün sistemə daxil edildi.",
                      "success"
                    );
                  } catch (err) {
                    showPopup(
