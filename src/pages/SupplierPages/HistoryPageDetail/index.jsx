...
const OrderHistoryDetailSuplier = () => {
  ...
  const uniqueCategories = [
    ...new Set(
      orderData?.items
        ?.map((item) => item.product?.categoryName)
        .filter(Boolean)
    ),
  ];
  ...
  const filtered =
    orderData?.items
      ?.map((item) => {
        ...
        const provided = `${item.suppliedQuantity} ${
          item.product?.measure || ""
        }`;
        ...
        const received =
          item.orderItemDeliveryTime === "01.01.0001"
            ? "—"
            : item.orderItemDeliveryTime;
  ...
  const isMobile = window.innerWidth <= 768;
    ...
    <div className="order-history-detail-main-supplier">
      <div className="order-history-detail-supplier">
        <h2>
          <NavLink className="link" to="/supplier/history">
            — Tarixçə
          </NavLink>{" "}
          — Sifariş detalları
        </h2>
        <div key={orderData?.id} className="order-history-detail-supplier_item">
          <div className={"techizat"}>
            <div className={"order-history-detail-supplier__ids"}>
              <p className="order-history-detail-supplier__id">
                <span>Təchizatçının adı:</span> {orderData?.fighterInfo?.name}{" "}
                {orderData?.fighterInfo?.surname}
              </p>
              <p className="order-history-detail-supplier__id">
                <span>Sifarişçinin adı:</span> {orderData?.adminInfo?.name}{" "}
                {orderData?.adminInfo?.surname}
              </p>
            </div>
          </div>
          <div className="order-history-detail-supplier__details">
            <div className="order-history-detail-supplier__ids">
              <p className="order-history-detail-supplier__id">
              ...
              <p className="order-history-detail-supplier__id">
                ...
                {`${orderData?.items
                  ?.reduce(
                    (sum, item) =>
                      sum + item.suppliedQuantity * (item?.price || 0),
                    0
                  )
                  .toFixed(2)} ₼`}
              </p>
            ...
            {isMobile ? (
              ...
              <div
                className={`order-history-detail-supplier__status ${
                  status === "Tamamlanmış"
                    ? "completed"
                    : status === "Sifarişçidən təhvil gözləyən"
                    ? "pending"
      ...
      {isOverheadModalOpen && (
        <div
          className="overhead-modal-overlay"
      ...
      {selectedOverheadImage && (
        <div
          ...
          onClick={() => setSelectedOverheadImage(null)}
        >
          <div
