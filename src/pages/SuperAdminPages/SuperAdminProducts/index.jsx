...
const SuperAdminProducts = () => {
  ...
  const { data: getProductAddPending, refetch: addPendingRefetch } =
    useGetProductAddPendingQuery();
  const addRequests = getProductAddPending?.data;
  const { data: getProductDeletePending, refetch: deletePendingRefetch } =
    useGetProductDeletePendingQuery();
  ...
  const currentDataSet =
    activeTab === "requests" ? combinedRequests : filteredProducts || [];
                    ...
                    const handleConfirm = async () => {
                      try {
                        if (item.statusType === "add") {
                          ...
                          showPopup(
                            "Məhsul yaratma tələbini təsdiq etdiniz",
                            "Yeni məhsul sistemə əlavə olundu",
                            "success"
                          );
                        } else if (item.statusType === "delete") {
                          await confirmDelete(item.id);
                          showPopup(
                            "Məhsulun silinmə tələbini təsdiq etdiniz",
                            "Seçilmiş məhsul sistemdən silindi",
                            "success"
                          );
                        }
                        addPendingRefetch();
                    ...
                    const handleReject = async () => {
                      try {
                        if (item.statusType === "add") {
                          ...
                          showPopup(
                            "Məhsul yaratma tələbini ləğv etdiniz",
                            "Məhsul yaradılması ilə bağlı tələb silindi",
                            "success"
                          );
                        } else if (item.statusType === "delete") {
                          await rejectDelete(item.id);
                          showPopup(
                            "Məhsulun silinmə tələbini ləğv etdiniz",
                            "Məhsul sistemdə saxlanıldı",
                            "success"
                          );
                        }
                        addPendingRefetch();
