...
const SuperAdminCategories = () => {
  ...
  //Sorgular
  const { data: getAllCategories, refetch: categoryRefetch } =
    useGetAllCategoriesQuery();
  const {
    data: searchResults,
  ...
  } = useGetAllCategoriesBySearchQuery(searchName, {
    skip: !searchName,
  });
  const categories = searchName
    ? searchResults?.data || []
  ...
  const { data: getCategorieAddPending, refetch: addPendingRefetch } =
    useGetCategorieAddPendingQuery();
  const addRequests = getCategorieAddPending?.data;
  const { data: getCategorieDeletePending, refetch: deletePendingRefetch } =
    useGetCategorieDeletePendingQuery();
  ...
  const currentDataSet =
    activeTab === "requests" ? combinedRequests : filteredCategories || [];

  const { data: getCategorieUpdatePending, refetch: editRefetch } =
    useGetCategorieUpdatePendingQuery();
      ...
      <div className="super-admin-category">
        <div className={"headerr"}>
          <div className={"head"}>
            <h2>Kateqoriyalar</h2>
            <p>
              Buradan kateqoriyaları idarə edə və yenilərini yarada bilərsiniz.
            </p>
          </div>
          <div>
                    ...
                    const handleConfirm = async () => {
                      try {
                        if (item.statusType === "add") {
                          ...
                          showPopup(
                            "Kateqoriya yaratma tələbini təsdiq etdiniz",
                            "Yeni kateqoriya sistemə əlavə olundu",
                            "success"
                          );
                        } else if (item.statusType === "delete") {
                          await confirmDelete(item.id``);
                          showPopup(
                            "Kateqoryanın silinmə tələbini təsdiq etdiniz",
                            "Seçilmiş kateqoriya sistemdən silindi",
                            "success"
                          );
                        }
                        addPendingRefetch();
                    ...
                    const handleReject = async () => {
                      try {
                        if (item.statusType === "add") {
                          ...
                          showPopup(
                            "Kateqoriya yaratma tələbini ləğv etdiniz",
                            "Kateqoriya yaradılması ilə bağlı tələb silindi",
                            "success"
                          );
                        } else if (item.statusType === "delete") {
                          await rejectDelete(item.id);
                          showPopup(
                            "Kateqoriyanın silinmə tələbini ləğv etdiniz",
                            "Kateqoriya sistemdə saxlanıldı",
                            "success"
                          );
                        }
                        addPendingRefetch();
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  setModalData(null);
                  showPopup(
                    "Kateqoriyaya uğurla düzəliş etdiniz",
