...
const OrderHistoryDetailSuperAdmin = () => {
    ...
    const filtered = orderData?.items
        ?.filter((item) => {
            ...
            return (
                (!searchName || name.includes(searchNameLower)) &&
                (!searchCategory || category.includes(searchCategoryLower))
            );
        })
        ?.map((item) => {
            ...
            return {
                ...
                provided: `${item.suppliedQuantity} ${item.product?.measure || ''}`,
                priceEach: `${(item?.price || 0).toFixed(2)} ₼`,
                price: `${(item.suppliedQuantity * (item?.price || 0)).toFixed(2)} ₼`,
                created: orderData?.createdDate,
                delivery: orderData?.orderLimitTime,
                received: item.orderItemDeliveryTime === '01.01.0001' ? '—' : item.orderItemDeliveryTime,
